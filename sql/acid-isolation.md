# Database Transaction Isolation: A Complete Guide

## Introduction
This document provides a comprehensive guide to database transaction isolation levels, their associated problems, and solutions. We'll use an **Online Store (Amazon-style)** scenario with a `Product` table containing `ID: 1, Name: iPhone, Stock: 10` as our running example throughout all explanations.
...
### SQL Example for Problem 1: Dirty Read

Here is the step-by-step SQL simulation of the **Dirty Read** anomaly. To understand this deeply, imagine you have two open terminal windows (Connection A and Connection B) talking to the same database.
...
| 5 | 00:05 | | `SELECT stock FROM Product WHERE id = 1;` | **DIRTY READ!**<br>The database returns **20**. The customer thinks there are 20 phones. |
...
| 4 | 00:04 | | `BEGIN TRANSACTION;` | Customer logs in. |
| 5 | 00:05 | | `SELECT stock FROM Product WHERE id = 1;` | **PROTECTION!**<br>The DB returns **10** (the last committed value).<br>**Note:** In some databases (e.g., SQL Server), this query may **pause and wait** until Transaction A finishes. |
| 6 | 00:06 | `COMMIT;` | | Manager saves the work. Stock is officially **20**. |
| 7 | 00:07 | | `SELECT stock FROM Product WHERE id = 1;` | The customer now sees **20**. |
...
### SQL Example: Non-Repeatable Read
...
| 7 | 00:07 | `SELECT stock FROM Product WHERE id = 1;` | | **NON-REPEATABLE READ!**<br>The Accountant sees **5**. |
...
| 7 | 00:07 | `SELECT stock FROM Product WHERE id = 1;` | | **PROTECTION!**<br>The Accountant *still* sees **10**. |
...
| 7 | 00:07 | `SELECT COUNT(*) FROM Orders WHERE status = 'PENDING';` | | **PHANTOM READ!**<br>Manager sees **3**. |
...
| 4 | 00:04 | | `BEGIN;` | |
| 5 | 00:05 | | `INSERT INTO Orders ... 'PENDING';` | **BLOCKING!**<br>Transaction B **waits** (freezes). The DB won't let a new 'PENDING' order be added because it would change A's results. |
| 6 | 00:06 | `COMMIT;` | | Manager finishes. |
| 7 | 00:07 | | *(System unfreezes B)*<br>`COMMIT;` | Now the customer's order is finally saved. |
...
### SQL Example for Problem 4: Write Skew

Let's tackle **Write Skew** first. This is the "final boss" of database isolation. It is the most dangerous because **Repeatable Read** (the level that solves all our previous problems) cannot stop it. Only **Serializable** can.
...
## Summary Checklist and Recommendations
...
4. **Write Skew** (Separate updates breaking a global rule) → Fix: **Serializable**.
